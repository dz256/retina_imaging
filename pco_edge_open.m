function [ errorCode, out_ptr, sBufNr, im_ptr, ev_ptr ] = pco_edge_open(exposure_time)
...
set_default = 0;
[errorCode,glvar] = pco_edge_setup(set_default,glvar,exposure_time);
...
if ~isempty(strfind(text,'edge'))
    ...
    if(errorCode)
        pco_errdisp('pco_edge_open',errorCode);
    else
        disp('Edge open');
    end
